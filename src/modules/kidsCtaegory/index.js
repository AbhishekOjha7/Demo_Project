...
export default function MenCategory() {
  ...
  const {Kids_Section} = useSelector(store => store.HomeReducer);

  useEffect(() => {
    dispatch(getHomeAction('kids'));
  }, []);

  const renderItems = ({item}) => {
    if (item.type == 'circleItemSlider') {
      return <CircleComponent src={Kids_Section
  } />;
      ...
      item.type === 'banner' &&
      item.items.tag == 'FreshStyle-header-June'
    ) {
      return (
        <Image
          source={{uri: item.items[0].url}}
      ...
      item.tag == 'FreshStyle-Slider-May'
    ) {
      ...
      item.type == 'banner' &&
      item.tag == 'Campaign spot-Eoss Extra 20%'
    ) {
      return (
        <Image
          source={{uri: item.items[0].url}}
    ...
    else if (item.type == 'grid' && item.tag == 'SBC-Grid-June-BAU') {
      return (
    ...
    else if (item.type == 'grid' && item.tag == 'Ramadan-Shop By Age') {
      return(

        <View>
   <Text style={{marginTop:20,marginLeft:10}}>{item.header.title}</Text>
<GridComponent
...
/>
      ...
      item.type == 'banner' &&
      item.tag == 'Campaign Spot-Character Mania-New'
    ) {
      return (
        <>
      ...
      item.type == 'bannerSliderWithLabel' &&
      item.tag == 'Shop by Clothing size-Slider'
    ) {
      return<BannerSlider data={item.items} 
        height={120}
        width={150}
      ...
      return(
        ...
        <View>
        <Text style={{}}>{item.header.title}</Text>
          <KidsGrid
          data={item.items}
          />
        </View>
